...
var handleNicknameChangeRequestSuccess =
    function (nick, nicknames, socket, io, room, isDefault) {
  ...
  io.in(room).emit('message', {text: message});
}

var handleNicknameChangeRequest =
    function (data, nicknames, socket, io, isDefault) {
  ...
  if (nick === nicknames[socket.id]) {
    ...
    var room = data.room;
    handleNicknameChangeRequestSuccess(nick, nicknames, socket, io, room, isDefault);
  }
};

var joinRoom = function (socket, room, currentRooms) {
  socket.join(room);
  currentRooms[socket.id] = room;
};

var handleRoomChangeRequest = function (data, currentRooms, socket) {
  socket.leave(currentRooms[socket.id]);
  joinRoom(socket, data.newRoom, currentRooms);
...
var createChat = function (server) {
  ...
  io.on('connection', function (socket) {
    joinRoom(socket, 'lobby', currentRooms);

    var defaultNick = "guest" + guestNumber;
    handleNicknameChangeRequest({
      proposedNick: defaultNick, room: 'lobby'
    }, nicknames, socket, io, true);
    guestNumber += 1;
    ...
    socket.on('nicknameChangeRequest', function (data) {
      handleNicknameChangeRequest(data, nicknames, socket, io);
    });

    socket.on('roomChangeRequest', function (data) {
      handleRoomChangeRequest(data, currentRooms, socket);
    });
